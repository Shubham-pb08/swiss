let recipes = [{

    chickenribs: [
        {
            item_name: 'CHICKEN GLAZING PROCEDURES',
            item_desc: {
                item_ing1: {
                    name: "Chicken",
                    qnt: "1 each"
                },
                item_ing2: {
                    name: "Glaze Choice: Honey Garlic OR Thai Sweet & Spicy",
                    qnt: "0.5 oz (14 g) 1/4 Chicken \n 1 oz (28 g) Double Leg \n 1.3 oz (36 g) 1/2 Chicken"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'QUARTER CHICKEN DINNER',
            item_desc: {
                item_ing1: {
                    name: "Quarter Chicken",
                    qnt: "1 leg"
                },
                item_ing2: {
                    name: "Dinner Roll ",
                    qnt: "1 each"
                },
                item_ing3: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing4: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing5: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'QUARTER CHICKEN DINNER -WHITE',
            item_desc: {
                item_ing1: {
                    name: "Quarter Chicken",
                    qnt: "1 breast"
                },
                item_ing2: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing3: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing4: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing5: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'HALF CHICKEN DINNER',
            item_desc: {
                item_ing1: {
                    name: "Half Chicken ",
                    qnt: "1 breast & 1 leg"
                },
                item_ing2: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing3: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing4: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing5: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'DOUBLE LEG DINNER ',
            item_desc: {
                item_ing1: {
                    name: "Quarter Chicken",
                    qnt: "2 legs"
                },
                item_ing2: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing3: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing4: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing5: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'CHICKEN & THIRD RACK COMBO',
            item_desc: {
                item_ing1: {
                    name: "Ribs",
                    qnt: "⅓ Rack"
                },
                item_ing2: {
                    name: "Choice of Smoky BBQ Sauce OR Honey Garlic",
                    qnt: "1 fl oz (30ml)"
                },
                item_ing3: {
                    name: "Quarter Chicken ",
                    qnt: "1 breast or leg"
                },
                item_ing4: {
                    name: "Dinner Roll ",
                    qnt: "1 each"
                },
                item_ing5: {
                    name: "Side Choice ",
                    qnt: "1 portion"
                },
                item_ing6: {
                    name: "Creamy Coleslaw",
                    qnt: "2 oz (56 g)"
                },
                item_ing7: {
                    name: "Butter Pot",
                    qnt: "1 each"
                },
                item_ing8: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing9: {
                    name: "Rib Sauce Portion Pack (Off Premise only)",
                    qnt: "1 each"
                },
                item_ing10: {
                    name: "Glaze Choice: Honey Garlic OR Thai Sweet & Spicy (Optional)",
                    qnt: "0.5 oz (14 g) (On Chicken) And 2 oz (57 g) (Served on side)"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: '1/2 RACK BBQ RIB DINNER',
            item_desc: {
                item_ing1: {
                    name: "Back Ribs",
                    qnt: "½ Rack"
                },
                item_ing2: {
                    name: "Choice of Rib Sauce",
                    qnt: "1 fl oz (30ml)"
                },
                item_ing3: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing4: {
                    name: "Side Choice ",
                    qnt: "1 portion"
                },
                item_ing5: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                },
                item_ing6: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                }
            },
            item_img: "img/potpie.png"
        },{
            item_name: 'FULL RACK BBQ RIB DINNER',
            item_desc: {
                item_ing1: {
                    name: "Back Ribs",
                    qnt: "2 X ½ Rack"
                },
                item_ing2: {
                    name: "Choice of Rib Sauce",
                    qnt: "2 fl oz (60ml)"
                },
                item_ing3: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing4: {
                    name: "Side Choice ",
                    qnt: "1 portion"
                },
                item_ing5: {
                    name: "Creamy Coleslaw ",
                    qnt: "2 oz (56 g)"
                },
                item_ing6: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                },
                item_ing7: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                }
            },
            item_img: "img/potpie.png"
        },{
            item_name: '¼ CHICKEN & 1/2 RACK BBQ RIB DINNER',
            item_desc: {
                item_ing1: {
                    name: "Back Ribs",
                    qnt: "1/2 Rack"
                },
                item_ing2: {
                    name: "Choice of Rib Sauce",
                    qnt: "1 fl oz (30ml)"
                },
                item_ing3: {
                    name: "Quarter Chicken",
                    qnt: "1 breast or leg"
                },
                item_ing4: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing5: {
                    name: "Side Choice ",
                    qnt: "1 portion"
                },
                item_ing6: {
                    name: "Creamy Coleslaw ",
                    qnt: "2 oz (56 g)"
                },
                item_ing7: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                },
                item_ing8: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing9: {
                    name: "Glaze Choice: Honey Garlic OR Thai Sweet & Spicy (Optional)",
                    qnt: "0.5 oz (14 g) (On Chicken) And 2 oz (57 g) (Served on side)"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'RIB & WING COMBO',
            item_desc: {
                item_ing1: {
                    name: "Chicken Wings, Prepared",
                    qnt: "½ portion(5 pieces)"
                },
                item_ing2: {
                    name: "Choice of Wing Sauce",
                    qnt: "1.5 fl oz (45ml)"
                },
                item_ing3: {
                    name: "Back Ribs",
                    qnt: "1/2 Rack"
                },
                item_ing4: {
                    name: "Choice of Rib Sauce ",
                    qnt: "1 fl oz (30ml)"
                },
                item_ing5: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing6: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing7: {
                    name: "Creamy Coleslaw",
                    qnt: "2 oz (56 g)"
                },
                item_ing8: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing9: {
                    name: "Butter Pot",
                    qnt: "1 each"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'FAMILY PAK',
            item_desc: {
                item_ing1: {
                    name: "Whole Chicken ",
                    qnt: "4 Quarters"
                },
                item_ing2: {
                    name: "Dinner Rolls",
                    qnt: "4 each"
                },
                item_ing3: {
                    name: "Side Choice",
                    qnt: "4 portion"
                },
                item_ing4: {
                    name: "Chalet Sauce",
                    qnt: "12 fl oz (360 ml)"
                },
                item_ing5: {
                    name: "Glaze Choice: Honey Garlic OR Thai Sweet & Spicy (Optional)",
                    qnt: "2.6 fl oz (72 g) (on Chicken) Or 8 fl oz (227 g) (on the side)"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'CHICKEN & RIBS FAMILY PAK',
            item_desc: {
                item_ing1: {
                    name: "Whole Chicken",
                    qnt: "4 Quarters"
                },
                item_ing2: {
                    name: "Back Ribs",
                    qnt: "½ Rack"
                },
                item_ing3: {
                    name: "Choice of Rib Sauce",
                    qnt: "1 fl oz (30ml)"
                },
                item_ing4: {
                    name: "Dinner Rolls",
                    qnt: "4 each"
                },
                item_ing5: {
                    name: "Side Choice",
                    qnt: "4 portion"
                },
                item_ing6: {
                    name: "Creamy Coleslaw",
                    qnt: "4 oz (113 g)"
                },
                item_ing7: {
                    name: "Chalet Sauce",
                    qnt: "12 fl oz (360 ml)"
                },
                item_ing8: {
                    name: "Glaze Choice: Honey Garlic OR Thai Sweet & Spicy (Optional)",
                    qnt: "2.6 fl oz (72 g) (on Chicken) Or 8 fl oz (227 g) (on the side)"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'CHICKEN & WING FAMILY PACK',
            item_desc: {
                item_ing1: {
                    name: "Chicken Wings, Prepared",
                    qnt: "1 portion (10 pieces)"
                },
                item_ing2: {
                    name: "Choice of wing Sauce",
                    qnt: "2 fl oz (60 ml)"
                },
                item_ing3: {
                    name: "Whole Chicken",
                    qnt: "4 Quarters"
                },
                item_ing4: {
                    name: "Dinner Rolls",
                    qnt: "4 each"
                },
                item_ing5: {
                    name: "Side Choice",
                    qnt: "4 portion"
                },
                item_ing6: {
                    name: "Chalet Sauce",
                    qnt: "12 fl oz (360 ml)"
                },
                item_ing7: {
                    name: "Glaze Choice: Honey Garlic OR Thai Sweet & Spicy (Optional)",
                    qnt: "2.6 fl oz (72 g) (on Chicken) Or 8 fl oz (227 g) (on the side)"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'WINGS & RIBS',
            item_desc: {
                item_ing1: {
                    name: "Chicken Wings, Prepared",
                    qnt: "1 OR 2 portions (10 or 20 pieces)"
                },
                item_ing2: {
                    name: "Choice of Wing Sauce",
                    qnt: "2 fl oz (60 ml) OR 4 fl oz (120 ml)"
                },
                item_ing3: {
                    name: "Back Ribs",
                    qnt: "2 x ½ Racks"
                },
                item_ing4: {
                    name: "BBQ Sauce",
                    qnt: "2 fl oz (60 ml)"
                }
            },
            item_img: "img/potpie.png"
        },
    ],
    entree: [{
            item_name: 'POT PIE DINNER',
            item_desc: {
                item_ing1: {
                    name: "POT PIE",
                    qnt: "13 oz"
                },
                item_ing2: {
                    name: "Puff Pastry Top, Prepped",
                    qnt: "1 each"
                },
                item_ing3: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing4: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing5: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing6: {
                    name: "Butter Pot",
                    qnt: "1 portion"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'STIR FRY ENTREE',
            item_desc: {
                item_ing1: {
                    name: "Canola Oil",
                    qnt: "1 fl oz (30 ml)"
                },
                item_ing2: {
                    name: "Stir Fry Veg Pack",
                    qnt: "1 Pack"
                },
                item_ing3: {
                    name: "Chicken, peeled Meat pack",
                    qnt: "4 oz (114 g)"
                },
                item_ing4: {
                    name: "Teriyaki Sauce",
                    qnt: "3 fl oz (85 ml)"
                },
                item_ing5: {
                    name: "Stir Fry Rice Pilaf, Portion ",
                    qnt: "7 oz (198 g)"
                },
                item_ing6: {
                    name: "Add Spring rolls (Upon request, Dine In Only)",
                    qnt: "2 each"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: '5PC CRISPY CHICKEN TENDER ENTREE',
            item_desc: {
                item_ing1: {
                    name: "Chicken tenders",
                    qnt: "5 pieces"
                },
                item_ing2: {
                    name: "Choice of Dipping Sauce",
                    qnt: "1 fl oz (30ml) "
                },
                item_ing3: {
                    name: "Chalet Seasoning (Optional)",
                    qnt: "4 g"
                },
                item_ing4: {
                    name: "Dinner Roll",
                    qnt: "1 each"
                },
                item_ing5: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing6: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                },
                item_ing7: {
                    name: "Butter Pot ",
                    qnt: "1 each"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'CHICKEN WING MEAL - 8 OR 12 PIECES',
            item_desc: {
                item_ing1: {
                    name: "Chicken Wings, Prepared",
                    qnt: "8 or 12 Pieces "
                },
                item_ing2: {
                    name: "Chalet Seasoning (Optional)",
                    qnt: "8 g (8 pcs) OR 12 g (12 pcs)"
                },
                item_ing3: {
                    name: "Sauce (Optional)",
                    qnt: "2 fl oz (8) OR 3 fl oz (12)"
                },
                item_ing4: {
                    name: "ColeslawDinner Roll",
                    qnt: "2 oz portion"
                },
                item_ing5: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing6: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                }
            },
            item_img: "img/potpie.png"
        },
        {
            item_name: 'CHICKEN QUESADILLA',
            item_desc: {
                item_ing1: {
                    name: "Chicken Quesadilla",
                    qnt: "1 each"
                },
                item_ing2: {
                    name: "Garlic Spread",
                    qnt: "0.5 oz (14 g)"
                },
                item_ing3: {
                    name: "Salsa",
                    qnt: "1.5 oz (42 g)"
                },
                item_ing4: {
                    name: "Sour Cream",
                    qnt: "1.5 oz (42 g)"
                },
                item_ing5: {
                    name: "Side Choice",
                    qnt: "1 portion"
                },
                item_ing6: {
                    name: "Chalet Sauce",
                    qnt: "4 fl oz (120 ml)"
                }
            },
            item_img: "img/potpie.png"
        }
    ],
    sbw: [
        {
            item_name: 'WINGS & RIBS',
            item_desc: {
                item_ing1: {
                    name: "Chicken Wings, Prepared",
                    qnt: "1 OR 2 portions (10 or 20 pieces)"
                },
                item_ing2: {
                    name: "Choice of Wing Sauce",
                    qnt: "2 fl oz (60 ml) OR 4 fl oz (120 ml)"
                },
                item_ing3: {
                    name: "Back Ribs",
                    qnt: "2 x ½ Racks"
                },
                item_ing4: {
                    name: "BBQ Sauce",
                    qnt: "2 fl oz (60 ml)"
                }
            },
            item_img: "img/potpie.png"
        }
    ]

}];
